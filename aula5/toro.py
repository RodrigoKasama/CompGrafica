...
def toro(): 
    ...
    posicao = geometriaToro.toro(1, 4, 500, 500)
...
def desenha():
    ...
    camera = glm.lookAt(glm.vec3(0,2,14),glm.vec3(0,0,0),glm.vec3(0,1,0))
    model = glm.rotate(a,glm.vec3(-1,1,0))
    mvp = projection * camera * model

    GL.glBindVertexArray(torpVAO)
    GL.glUseProgram(progId)
    GL.glUniformMatrix4fv(GL.glGetUniformLocation(progId, "mvp"),1,GL.GL_FALSE,glm.value_ptr(mvp))
    GL.glDrawArrays(GL.GL_POINTS,0, 500*500)
...
def main():
    ...
    window = glfw.create_window(800, 600, "Paraboloide", None, None)
    if not window:
        glfw.terminate()
        return
